...
const DOCUMENT_TYPES = [
  { key: 'bonafide_letter_url', label: 'Bonafide Letter' },
  { key: 'internship_offer_letter_url', label: 'Offer Letter' },
  { key: 'internship_completion_letter_url', label: 'Completion Certificate' },
  { key: 'internship_report_url', label: 'Internship Report' },
];
...
export default function GuideDashboard() {
  ...
  async function fetchInternships(studentEmail) {
    ...
    try {
      const encodedEmail = encodeURIComponent(studentEmail);
      const res = await axios.get(`http://localhost:5000/api/guidefunctions/internship_with_doc/${encodedEmail}`);
      setInternships(res.data.internships || []);
    } catch {
      setError('Failed to fetch internships');
      setInternships([]);
            ...
            >
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                {DOCUMENT_TYPES.map(({ key, label }) => {
                  const url = internship.documents?.[key];
                  const exists = !!url;
                  return (
                    <button
                      key={key}
                      ...
                      aria-label={`${label} ${exists ? 'available' : 'not available'}`}
                      type="button"
                    >
                      {label}
                    </button>
